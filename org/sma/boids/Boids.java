...
public class Boids implements Iterable<Boid> {
	...
	public Boids(int n, int minx, int miny, int maxx, int maxy, int max_speed, Color color) {
		...
		for (int i = 0; i < n; i++)
			this.boids.add(new Boid((int) (Math.random() * (maxx - minx)) + minx,
					(int) (Math.random() * (maxy - minx)) + minx, (int) (Math.random() * max_speed),
					(int) (Math.random() * max_speed), this.color, max_speed));
	...
	/**
	 * Implémente la somme des forces dans le PFD Permet lors de l'héritage, de
	 * rajouter une nouvelle force sans réimplémenter nextStep.
	 * 
	 * @param b
	 * @return
	 */
	public Vect sumForce(Boid b) {
		return cohesion(b).translate(alignment(b)).translate(separation(b)).translate(screenKeeper(b))
				.translate(noFriend(b));

	}

	/**
	 * Implémente le pas suivant. La simulation réalise un PFD sur chaque boid à
	 * partir des données de ce qui va devenir l'ancien pas.
	 */
	...
	public Vect cohesion(Boid b) {
		...
		for (Iterator<Boid> i = this.getNeighbors(b, this.distanceCohesion * this.distanceCohesion); i
				.hasNext(); nbVois++)
	...
	public Vect alignment(Boid b) {
		...
		for (Iterator<Boid> i = this.getNeighbors(b, this.distanceAlignment * this.distanceAlignment); i
				.hasNext(); nbVois++)
	...
	public Vect separation(Boid b) {
		...
		for (Iterator<Boid> i = this.getNeighbors(b, this.distanceSeparation * this.distanceSeparation); i
				.hasNext(); nbVois++) {
	...
	/**
	 * Si b sort de l'écran, screenKeeper va retourner une force suffisamment
	 * grande pour lui permettre de revenir dans l'écran.
	 * 
	 * @param b
	 * @return
	 */
	public Vect screenKeeper(Boid b) {
		...
		Vect force = new Vect(
				(x < minx + this.seuilKeeper * (maxx - minx) ? minx + this.seuilKeeper * (maxx - minx) - x : 0)
						- (x > maxx - this.seuilKeeper * (maxx - minx) ? x - maxx + this.seuilKeeper * (maxx - minx)
								: 0),
				(y < miny + this.seuilKeeper * (maxy - miny) ? miny + this.seuilKeeper * (maxy - miny) - y : 0)
						- (y > maxy - this.seuilKeeper * (maxy - miny) ? y - maxy + this.seuilKeeper * (maxy - miny)
								: 0));
	...
	/**
	 * Si b n'a pas de voisin visible, noFriend va retourner une force
	 * perpendiculaire à sa vitesse, lui permettant de tourner.
	 * 
	 * @param b
	 * @return
	 */
	public Vect noFriend(Boid b) {
		int nbVois = -1;
		for (Iterator<Boid> i = this.getNeighbors(b, this.distanceNoFriend * this.distanceNoFriend); i
				.hasNext(); nbVois++)
			i.next();
